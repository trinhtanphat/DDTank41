...
Require ($handler -match 'legacyIsBand\s*=\s*packet\.ReadBoolean\(\)') 'server must preserve legacy isBand field'
Require ($handler -match 'paymentMode\s*=\s*packet\.ReadByte\(\)') 'server must read payment mode only after legacy isBand'
